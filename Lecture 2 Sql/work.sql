...
FROM BOOK_LEND_RECORD AS blr 
	...
	INNER JOIN BOOK_CLASS AS bc--或是用RIGHT JOIN，但是他這題是數個數，不用考慮到有些class底下的書可能沒有被借過
...
SET @USERID = (SELECT mm.[USER_ID] FROM MEMBER_M mm WHERE mm.USER_CNAME = '李四');
--@USERID = '0002'
--PRINT @USERID;
BEGIN TRY
	BEGIN TRAN
...
BEGIN TRY
	BEGIN TRAN
		...
		UPDATE BOOK_DATA
		SET BOOK_STATUS = 'A',--可借出
			BOOK_KEEPER = (SELECT mm.[USER_ID] FROM MEMBER_M mm WHERE mm.USER_CNAME = '李四'),--@USERID
			MODIFY_DATE = GETDATE(),
			MODIFY_USER = '0002'
		WHERE BOOK_ID = '2294'
		COMMIT TRAN
